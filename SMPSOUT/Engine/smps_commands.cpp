...
static void DoCoordinationFlag(TRK_RAM* Trk, const CMD_FLAGS* CFlag)
{
	...
	case CF_SND_CMD:
		...
		break;
	case CF_PLAY_DAC:
		//SmpsRAM._1C3C = Data[0x00];
		TempByt = CFlag->SubType & 0x0F;
		DAC_Play(TempByt, (Data[0x00] & 0x7F) - 0x01);
		if (Trk->SmpsCfg->DrumChnMode == DCHNMODE_CYMN && TempByt == 0x00)
		{
			if (Data[0x00])
	...
	case CF_PAN_ANIM:	// E4 Pan Animation
		...
		else
		{
			Trk->PanAni.Type = Data[0x00];
			if (Trk->PanAni.Type)
				printf("Z80 Pan Ani: %02X %02X %02X %02X %02X\n", Data[0x00], Data[0x01], Data[0x02], Data[0x03], Data[0x04]);
	...
	case CF_SPC_FM3:		// FE FM3 Special Mode
		if (Trk->SmpsCfg->SeqData != Trk->SmpsCfg->FMDrums.Data)
